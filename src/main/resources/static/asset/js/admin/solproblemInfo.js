$(function () {
    ...
    layui.use(['table', 'layer', 'form', 'laypage', 'laydate','upload'], function () {
        ...
        table.render({
            ...
            ,url: '/solvedProblem/getAllSolvedProblemInfo' //数据接口
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'solvedProblemId', title: '问题ID', width:100, sort: true, fixed: 'left', totalRowText: '合计：'} //totalRow：true则代表这列数据要合计
                ,{field: 'subject', title: '题干',edit: 'text',templet: '.typecolor'}
                ,{field: 'knowledgePoints', title: '相关知识点',  sort: true, totalRow: true,edit: 'text'}
                ,{field: 'solvedTimesAndVisitTimes', title: '解决次数/访问次数', sort: true,edit: 'text'}
        ...
        table.on('toolbar(zq_table)', function(obj){
            ...
            switch(obj.event){
                case 'delAll':
                    ...
                    break;
                case 'add':
                    window.open("/download/solvedProblem-samples.xlsx");
                    layer.open({
                        title:"批量添加题目",
                        type: 1,
                        ...
                        content:
                            ...
                            '  <p>点击上传，或将文件拖拽到此处</p>\n' +
                            '</div>'+
                            '  <div class="" id="tipWords"> 请注意上传的题目信息的格式</div>'
                    });
                    //拖拽上传
                    upload.render({
                        elem: '#uploadFile'
                        ,url: '/solvedProblem/insertBatch',
                        accept: 'file',
            ...
            switch(layEvent){
                case 'detail':
                    ...
                    layer.open({
                        ...
                        fixed: false, //不固定
                        maxmin: true,
                        content: '<form class="layui-form" lay-filter="problemInfo" style="margin: 15px" >' +
                            ...
                            '  </div>\n' +
                            '  <div class="layui-form-item">\n' +
                            '    <label class="layui-form-label">回答正确人数</label>\n' +
                            '    <div class="layui-input-inline">\n' +
                    ...
                    form.render(null,'problemInfo');
                    $.ajax({
                        url:'/solvedProblem/getSolProblemById/'+data.solvedProblemId,
                        dataType:'json',
                        ...
                        success:function (res) {
                            ...
                            $("#problemId").val(res.data.solvedProblemId);
                            $("#subject").val(res.data.subject);
                            $("#analysis").val(res.data.analysis);
                            $("#kps").val(res.data.knowledgePoints);
                            $("#answerPersonNum").val(res.data.customUserSet.length);
                ...
                case 'del':
                    console.log(data);
                    layer.confirm('您确定删除id：'+data.solvedProblemId+'的数据吗？', {btn: ['确认','取消'],btn1:function (index){
                                $.ajax({
                                    url: "/solvedProblem/deleteById/"+data.solvedProblemId,
                                    type: "get",
                                    dataType: "json",
                                    contentType: "application/json;charset=UTF-8",
                                    //向后端传输的章节数据
